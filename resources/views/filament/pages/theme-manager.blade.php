...
>
    ...
    {{-- ═══════════════════════════════════════════════════════════════ --}}
    {{-- FAR-LEFT SLIM ICON SIDEBAR (~54px) --}}
    {{-- ═══════════════════════════════════════════════════════════════ --}}
    <div class="flex flex-col w-[54px] min-w-[54px] items-center py-3.5 justify-between select-none z-20" style="background: #0a0b12; border-right: 1px solid rgba(255,255,255,0.06);">
        <div class="flex flex-col items-center gap-2 w-full px-1.5">
            ...
            <template x-for="(section, index) in sections" :key="section.id">
                <div class="relative group w-full flex justify-center">
                    ...
                    {{-- Tooltip --}}
                    <div class="absolute left-[48px] top-1/2 -translate-y-1/2 px-2.5 py-1 rounded-md text-xs font-semibold whitespace-nowrap hidden group-hover:block z-[100] shadow-xl pointer-events-none" style="background: #181926; color: #f1f5f9; border: 1px solid rgba(255,255,255,0.1);" x-text="section.label"></div>
                </div>
            </template>
            ...
            <template x-for="(item, index) in comingSoon" :key="item.label">
                <div class="relative group w-full flex justify-center">
                    ...
                    <div class="absolute left-[48px] top-1/2 -translate-y-1/2 px-2.5 py-1 rounded-md text-xs font-semibold whitespace-nowrap hidden group-hover:block z-[100] shadow-xl pointer-events-none" style="background: #181926; color: #64748b; border: 1px solid rgba(255,255,255,0.06);" x-text="item.label + ' (Soon)'"></div>
                </div>
            </template>
        </div>
        ...
        <a href="{{ url('/admin') }}" class="w-9 h-9 rounded-xl flex items-center justify-center text-gray-400 hover:text-white hover:bg-white/5 transition-all group relative" title="Back to Admin">
            ...
            <div class="absolute left-[48px] top-1/2 -translate-y-1/2 px-2.5 py-1 rounded-md text-xs font-semibold whitespace-nowrap hidden group-hover:block z-[100] shadow-xl pointer-events-none" style="background: #181926; color: #f1f5f9; border: 1px solid rgba(255,255,255,0.1);">Back to Admin</div>
        </a>
    </div>

    {{-- ═══════════════════════════════════════════════════════════════ --}}
    {{-- COMPACT EDITING CONTROLS PANEL (290px wide) --}}
    {{-- ═══════════════════════════════════════════════════════════════ --}}
    <div class="flex flex-col w-[290px] min-w-[290px] max-w-[290px] relative z-10" style="background: #0d0e17; border-right: 1px solid rgba(255,255,255,0.06);">
        {{-- Section Header --}}
        ...
        <div class="flex-1 overflow-y-auto px-4 py-3 pb-24 theme-editor-form theme-scroll">
            {{ $this->form }}
        </div>

        {{-- PINNED BOTTOM ACTION BAR --}}
        <div class="absolute bottom-0 left-0 right-0 z-30 p-2.5" style="background: #0d0e17; border-top: 1px solid rgba(255,255,255,0.06);">
            <div class="flex items-center justify-between gap-1.5">
                <div class="flex items-center gap-1">
                    {{-- Discard Changes --}}
                    <button wire:click="discardChanges" class="w-8 h-8 rounded-lg flex items-center justify-center text-gray-400 hover:text-white transition-all hover:bg-white/5 border border-white/5" title="Discard Changes">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0 3.181 3.183a8.25 8.25 0 0 0 13.803-3.7M4.031 9.865a8.25 8.25 0 0 1 13.803-3.7l3.181 3.182" /></svg>
                    </button>
                    {{-- Import Theme --}}
                    <button class="w-8 h-8 rounded-lg flex items-center justify-center text-gray-400 hover:text-white transition-all hover:bg-white/5 border border-white/5" title="Import Theme">
                    ...
                    {{-- Export Theme --}}
                    <button class="w-8 h-8 rounded-lg flex items-center justify-center text-gray-400 hover:text-white transition-all hover:bg-white/5 border border-white/5" title="Export Theme">
                ...
                <button
                    ...
                    class="flex-1 flex items-center justify-center gap-1.5 py-1.5 px-2.5 rounded-lg font-bold text-xs text-white transition-all duration-200 hover:brightness-110 active:scale-95 shadow-md shadow-purple-900/30"
    ...
    {{-- ═══════════════════════════════════════════════════════════════ --}}
    {{-- EXPANSIVE LIVE PREVIEW AREA (>75% width) --}}
    {{-- ═══════════════════════════════════════════════════════════════ --}}
    <div class="flex-1 flex flex-col min-w-0" style="background: #06070a;">
        {{-- Preview Top Toolbar --}}
        <div class="flex items-center justify-between px-6 h-[46px] min-h-[46px] select-none" style="background: #0a0b12; border-bottom: 1px solid rgba(255,255,255,0.06);">
            {{-- Target Preview Page Selector --}}
            ...
            <div class="flex items-center gap-1 bg-black/40 rounded-lg p-0.5 border border-white/5">
                <button
                    ...
                    :class="device === 'desktop' ? 'bg-white/10 text-white font-semibold' : 'text-gray-400 hover:text-white'"
                ...
                <button
                    @click="device = 'tablet'"
                    :class="device === 'tablet' ? 'bg-white/10 text-white font-semibold' : 'text-gray-400 hover:text-white'"
                    class="flex items-center gap-1.5 px-3 py-1 rounded-md text-xs transition-all"
                >
                ...
                <button
                    @click="device = 'mobile'"
                    :class="device === 'mobile' ? 'bg-white/10 text-white font-semibold' : 'text-gray-400 hover:text-white'"
                    class="flex items-center gap-1.5 px-3 py-1 rounded-md text-xs transition-all"
                >
            ...
            {{-- Refresh --}}
            <button @click="refreshPreview()" class="flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium text-gray-400 hover:text-white hover:bg-white/5 transition-all">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0 3.181 3.183a8.25 8.25 0 0 0 13.803-3.7M4.031 9.865a8.25 8.25 0 0 1 13.803-3.7l3.181 3.182" /></svg>
                Refresh
            </button>
        </div>

        {{-- Preview Viewport Canvas (Fills available space directly) --}}
        <div class="flex-1 p-5 overflow-hidden flex items-center justify-center relative select-none">
            <div
                :class="{
                    'w-full h-full': device === 'desktop',
                    'w-[768px] h-[92%] max-h-[850px]': device === 'tablet',
                    'w-[390px] h-[88%] max-h-[750px]': device === 'mobile'
                }"
                class="rounded-2xl overflow-hidden shadow-[0_20px_60px_rgba(0,0,0,0.7)] border border-white/10 transition-all duration-300 flex flex-col bg-gray-950"
            >
                <iframe

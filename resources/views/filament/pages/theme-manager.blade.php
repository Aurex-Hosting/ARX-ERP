<style>
    /* Flatten Filament sections — clean, modern, no bulky card boxes */
...
<div
    x-data="{
        ...
        updateDimensions() {
            ...
            if (!wrapper) return;
            this.containerWidth = wrapper.clientWidth - 32;
            this.containerHeight = wrapper.clientHeight - 32;

            if (this.device === 'desktop') {
                const targetW = 1280;
                this.scale = Math.min(1, this.containerWidth / targetW);
...
>
    ...
    {{-- ═══════════════════════════════════════════════════════════════ --}}
    <div class="flex flex-col w-[60px] min-w-[60px] items-center py-4 justify-between select-none" style="background: #0d0e17; border-right: 1px solid rgba(255,255,255,0.06);">
        <div class="flex flex-col items-center gap-2 w-full px-2">
            ...
            <template x-for="(section, index) in sections" :key="section.id">
                <div class="relative group w-full flex justify-center">
                    ...
                    <div
                        x-show="activeSection === section.id"
                        class="absolute -left-2 top-1/2 -translate-y-1/2 w-[3px] h-6 rounded-r-full shadow-[0_0_8px_rgba(139,92,246,0.8)]"
                    ...
                    {{-- Tooltip --}}
                    <div class="absolute left-[54px] top-1/2 -translate-y-1/2 px-2.5 py-1 rounded-md text-xs font-semibold whitespace-nowrap hidden group-hover:block z-[100] shadow-xl pointer-events-none" style="background: #181926; color: #f1f5f9; border: 1px solid rgba(255,255,255,0.1);" x-text="section.label"></div>
                </div>
            </template>

            {{-- Divider --}}
            <div class="w-6 h-px my-1.5" style="background: rgba(255,255,255,0.08);"></div>

            {{-- Coming Soon Icons --}}
            <template x-for="(item, index) in comingSoon" :key="item.label">
                <div class="relative group w-full flex justify-center">
                    <button class="w-10 h-10 rounded-xl flex items-center justify-center text-gray-600 cursor-not-allowed opacity-50" disabled>
                    ...
                    <div class="absolute left-[54px] top-1/2 -translate-y-1/2 px-2.5 py-1 rounded-md text-xs font-semibold whitespace-nowrap hidden group-hover:block z-[100] shadow-xl pointer-events-none" style="background: #181926; color: #64748b; border: 1px solid rgba(255,255,255,0.06);" x-text="item.label + ' (Soon)'"></div>
                </div>
            </template>
        </div>

        {{-- Back to Admin Button --}}
        <a href="{{ url('/admin') }}" class="w-10 h-10 rounded-xl flex items-center justify-center text-gray-400 hover:text-white hover:bg-white/10 transition-all group relative" title="Back to Admin">
            ...
            <div class="absolute left-[54px] top-1/2 -translate-y-1/2 px-2.5 py-1 rounded-md text-xs font-semibold whitespace-nowrap hidden group-hover:block z-[100] shadow-xl pointer-events-none" style="background: #181926; color: #f1f5f9; border: 1px solid rgba(255,255,255,0.1);">Back to Admin</div>
        </a>
    </div>

    {{-- ═══════════════════════════════════════════════════════════════ --}}
    {{-- MIDDLE FORM CONTROLS PANEL --}}
    {{-- ═══════════════════════════════════════════════════════════════ --}}
    <div class="flex flex-col w-[350px] min-w-[320px] max-w-[380px] relative" style="background: #10111a; border-right: 1px solid rgba(255,255,255,0.06);">
        {{-- Section Header --}}
        <div class="px-5 py-4" style="border-bottom: 1px solid rgba(255,255,255,0.06);">
            <template x-for="section in sections" :key="section.id">
                <div x-show="activeSection === section.id" x-cloak>
                    <h2 class="text-base font-bold text-white tracking-tight" x-text="section.label"></h2>
                    <p class="text-xs text-gray-400 mt-0.5" x-text="section.subtitle"></p>
                </div>
            </template>
        </div>

        {{-- Scrollable Form Content --}}
        <div class="flex-1 overflow-y-auto px-5 py-4 pb-28 theme-editor-form theme-scroll">
            {{ $this->form }}
        </div>

        {{-- BOTTOM FLOATING SAVE BAR --}}
        <div class="absolute bottom-0 left-0 right-0 z-40 p-3" style="background: #10111a; border-top: 1px solid rgba(255,255,255,0.06);">
            <div class="flex items-center justify-between gap-2">
                <div class="flex items-center gap-1.5">
                    {{-- Discard Changes --}}
                    <button wire:click="discardChanges" class="w-9 h-9 rounded-lg flex items-center justify-center text-gray-400 hover:text-white transition-all hover:bg-white/5 border border-white/5" title="Discard Changes">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0 3.181 3.183a8.25 8.25 0 0 0 13.803-3.7M4.031 9.865a8.25 8.25 0 0 1 13.803-3.7l3.181 3.182" /></svg>
                    </button>
                    {{-- Import Theme --}}
                    <button class="w-9 h-9 rounded-lg flex items-center justify-center text-gray-400 hover:text-white transition-all hover:bg-white/5 border border-white/5" title="Import Theme">
                    ...
                    {{-- Export Theme --}}
                    <button class="w-9 h-9 rounded-lg flex items-center justify-center text-gray-400 hover:text-white transition-all hover:bg-white/5 border border-white/5" title="Export Theme">
                ...
                <button
                    ...
                    class="flex-1 flex items-center justify-center gap-2 py-2 px-3 rounded-lg font-bold text-xs text-white transition-all duration-200 hover:brightness-110 active:scale-95 shadow-md shadow-purple-900/30"
    ...
    {{-- ═══════════════════════════════════════════════════════════════ --}}
    <div class="flex-1 flex flex-col min-w-0" style="background: #06070a;">
        {{-- Preview Top Toolbar --}}
        <div class="flex items-center justify-between px-5 h-[46px] min-h-[46px] select-none" style="background: #0d0e17; border-bottom: 1px solid rgba(255,255,255,0.06);">
            {{-- Target Preview Page Switcher --}}
            <div class="flex items-center gap-2">
                ...
                <div class="flex items-center bg-black/40 rounded-lg p-0.5 border border-white/5">
                    <button 
                        ...
                        :class="previewPage === '/admin' ? 'bg-purple-600 text-white shadow' : 'text-gray-400 hover:text-white'" 
                    ...
                    <button 
                        ...
                        :class="previewPage === '/dashboard' ? 'bg-purple-600 text-white shadow' : 'text-gray-400 hover:text-white'" 
                        class="px-2.5 py-1 text-xs font-medium rounded-md transition-all"
                    >
                        Dashboard
                    </button>
                    <button 
                        ...
                        :class="previewPage === '/admin/login' ? 'bg-purple-600 text-white shadow' : 'text-gray-400 hover:text-white'" 
                        class="px-2.5 py-1 text-xs font-medium rounded-md transition-all"
                    >
                        Login
                    </button>
                    <button 
                        ...
                        :class="previewPage === '/404' ? 'bg-purple-600 text-white shadow' : 'text-gray-400 hover:text-white'" 
                        class="px-2.5 py-1 text-xs font-medium rounded-md transition-all"
                    >
                        404 Page
                    </button>
            ...
            <div class="flex items-center gap-1 bg-black/40 rounded-lg p-0.5 border border-white/5">
                <button
                    @click="device = 'desktop'; updateDimensions()"
                    :class="device === 'desktop' ? 'bg-white/10 text-white' : 'text-gray-400 hover:text-white'"
                ...
                <button
                    @click="device = 'tablet'; updateDimensions()"
                    :class="device === 'tablet' ? 'bg-white/10 text-white' : 'text-gray-400 hover:text-white'"
                    class="flex items-center gap-1.5 px-3 py-1 rounded-md text-xs font-medium transition-all"
                >
                ...
                <button
                    @click="device = 'mobile'; updateDimensions()"
                    :class="device === 'mobile' ? 'bg-white/10 text-white' : 'text-gray-400 hover:text-white'"
                    class="flex items-center gap-1.5 px-3 py-1 rounded-md text-xs font-medium transition-all"
                >
            ...
            {{-- Right Actions --}}
            <div class="flex items-center gap-2">
                <button @click="refreshPreview()" class="flex items-center gap-1.5 px-3 py-1 rounded-md text-xs font-medium text-gray-400 hover:text-white hover:bg-white/5 transition-all">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0 3.181 3.183a8.25 8.25 0 0 0 13.803-3.7M4.031 9.865a8.25 8.25 0 0 1 13.803-3.7l3.181 3.182" /></svg>
                    Refresh
                </button>
            </div>
        ...
        {{-- Preview Viewport Container with Dynamic Scaling --}}
        <div x-ref="previewWrapper" class="flex-1 flex items-center justify-center p-4 overflow-hidden relative select-none">
            <div
                class="rounded-xl overflow-hidden transition-all duration-300 shadow-2xl border border-white/10"
                :style="device === 'desktop' ? 'width: 1280px; height: 800px; transform: scale(' + scale + '); transform-origin: center center;' : (device === 'tablet' ? 'width: 768px; height: 900px; transform: scale(' + scale + '); transform-origin: center center;' : 'width: 375px; height: 667px; transform: scale(1); transform-origin: center center;')"
            >
                <iframe
                    id="theme-preview-iframe"
                    :src="previewPage"
                    class="w-full h-full border-0 bg-gray-950"
